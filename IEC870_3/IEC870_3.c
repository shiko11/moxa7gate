...
int read_iec3_data(char *in_buf, int in_len, kp_cfg_t *kp_cfg, iec3_large_out_buf_t *iec3_buf){
  ...
  while(i<in_len){
    ...
    case VAR_LEN_FRAME_TYPE:
      ...
      if(!pack_len){
	i += pack_len; //длина не нулевая продолжаем обработку 
      }
      else {
...
void send_ACK(kp_cfg_t *kp_cfg, iec3_large_out_buf_t *iec3_buf){ 
  ...
  if((kp_cfg->state & KP_CFG_ACK_MASK)== KP_CFG_SHORT_ACK){
    ...
    iec3_buf_wrt_from_begin(temp_strig, SHORT_ACK_LEN, iec3_buf);
...
void send_long_ACK(kp_cfg_t *kp_cfg, iec3_large_out_buf_t *iec3_buf){
  ...
  temp_strig[SERVICE_CODE_CFL] = SERVICE_CODE_ACK | PTP_MASK;// | ( kp_cfg->FCB*FCB_MASk);
  temp_strig[KP_ADDR_CFL] = kp_cfg->addr;
  temp_strig[CHECKSUM_CLF] = kp_cfg->addr + (SERVICE_CODE_ACK | PTP_MASK);
  temp_strig[END_OF_CFL] = END_OF_FRAME;
  //iec3_buf_wrt(temp_strig, CONSTANT_FRAME_LEN, iec3_buf);
  iec3_buf_wrt_from_begin(temp_strig, CONSTANT_FRAME_LEN, iec3_buf);
}
...
void send_NACK(kp_cfg_t *kp_cfg, iec3_large_out_buf_t *iec3_buf){ 
  ...
  if((kp_cfg->state & KP_CFG_ACK_MASK)== KP_CFG_SHORT_ACK){
    ...
    //длинный ответ
    send_long_NACK(kp_cfg, iec3_buf);
    #ifdef DEBUG_IEC870_3
    printf("Используется длинный тип пакета для потверждения\n ");
    #endif
...
void send_long_NACK(kp_cfg_t *kp_cfg, iec3_large_out_buf_t *iec3_buf){ 
  unsigned char temp_strig[5];
  temp_strig[START_BYTE_CFL] = CONST_LEN_FRAME_TYPE;
  temp_strig[SERVICE_CODE_CFL] = SERVICE_CODE_NACK | PTP_MASK;// | ( kp_cfg->FCB*FCB_MASk);
  temp_strig[KP_ADDR_CFL] = kp_cfg->addr;
  temp_strig[CHECKSUM_CLF] = kp_cfg->addr + (SERVICE_CODE_ACK | PTP_MASK);
  temp_strig[END_OF_CFL] = END_OF_FRAME;
  //iec3_buf_wrt(temp_strig, CONSTANT_FRAME_LEN, iec3_buf);
...
int  analize_var_frame(char *in_buf, int in_len, kp_cfg_t *kp_cfg, int *pack_len, iec3_large_out_buf_t *iec3_buf){
  ...
  *pack_len = pl;
  ...
  case GENERAL_POLL2:
    ...
    #endif
    pthread_mutex_lock(&kp_cfg->mutex_b);
    ...
    pthread_mutex_unlock(&kp_cfg->mutex_b);
    return 0;
  case SIGNAL_POLL:
  ...
  case SET_TIME:
    #ifdef DEBUG_IEC870_3
    printf("Пришла команда телесинхронихации\n");
    #endif
    /*быдлокод но сойдет копируем метку времени*/
    kp_cfg->time.tch[0] = in_buf[COMMAND_FELD + 1];
    kp_cfg->time.tch[1] = in_buf[COMMAND_FELD + 2];
    kp_cfg->time.tch[2] = in_buf[COMMAND_FELD + 3];
    kp_cfg->time.tch[3] = in_buf[COMMAND_FELD + 4];
  ...
  default: 
    #ifdef DEBUG_IEC870_3
    printf("неизвестная команда для пакета переменной длины: %Xh\n", in_buf[COMMAND_FELD]);
    #endif
...
int  analize_control_feld(char control_feld, kp_cfg_t *kp_cfg, iec3_large_out_buf_t *iec3_buf, int *code){
  if((control_feld & FCV_MASK) == FCV_MASK){
    ...
    if((control_feld & FCB_MASK) == FCB_MASK){
      kp_cfg->state |= KP_CFG_FCB_MASK;
    }
  }else{
...
void clear_send_buf(iec3_large_out_buf_t *iec3_buf){
  iec3_buf->begin = 0;
  iec3_buf->end = 0;
...
void write_state_connection(kp_cfg_t *kp_cfg, iec3_large_out_buf_t *iec3_buf){
  unsigned char temp_strig[5];
  temp_strig[START_BYTE_CFL] = CONST_LEN_FRAME_TYPE;
  temp_strig[SERVICE_CODE_CFL] =  SERVICE_CODE_CON_STAT | PTP_MASK;
  //temp_strig[SERVICE_CODE_CFL] = SERVICE_CODE_CON_STAT;
  temp_strig[KP_ADDR_CFL] = kp_cfg->addr;
  temp_strig[CHECKSUM_CLF] = kp_cfg->addr + (SERVICE_CODE_CON_STAT | PTP_MASK);
  temp_strig[END_OF_CFL] = END_OF_FRAME;
  //iec3_buf_wrt(temp_strig, CONSTANT_FRAME_LEN, iec3_buf);
  iec3_buf_wrt_from_begin(temp_strig, CONSTANT_FRAME_LEN, iec3_buf);
}

/*Функция записи в большой буфер передачи в конец*/
void iec3_buf_wrt(unsigned char *data, unsigned int len, iec3_large_out_buf_t *iec3_buf){
  unsigned int i, j;
  iec3_buf->len[iec3_buf->len_end] = len;
  #ifdef DEBUG_IEC870_3
  printf("\nSEND TO LARGE IEC BUFER DATA WITH LEN = %d | %d: ", len ,iec3_buf->len[iec3_buf->len_end]);
  #endif

  iec3_buf->len_end++;
  if(iec3_buf->len_end >= MAX_IEC_LARGE_OUT_BUF_LEN) iec3_buf->len_end = 0;

  for(i = 0, j = iec3_buf->end; i < len; i++, j++){
    if(j  >=  MAX_IEC_LARGE_OUT_BUF_LEN) {
      j = 0;
    }
...
void iec3_buf_wrt_from_begin(unsigned char *data, unsigned int len, iec3_large_out_buf_t *iec3_buf){
  unsigned int i, j;
  /*Смещаем начало буфера длины на одну позицию назад и записываем туда длину сообщения*/
  if(iec3_buf->len_begin == 0){
    iec3_buf->len_begin = MAX_IEC_LARGE_OUT_BUF_LEN - 1;
  }else{
    iec3_buf->len_begin--;
  }
  iec3_buf->len[iec3_buf->len_begin] = len;
  
  /*Смещаемся от начала буфера данных на величину сообщения и запоминаем эту позицию*/
  if(iec3_buf->begin >= len){
    j = iec3_buf->begin - len;
  }else{
    j = MAX_IEC_LARGE_OUT_BUF_LEN - (len - iec3_buf->begin - 1) - 1;
  }
  iec3_buf->begin = j;
  
  #ifdef DEBUG_IEC870_3
  printf("\nSEND TO LARGE IEC BUFER DATA WITH LEN = %d | %d: ", len ,iec3_buf->len[iec3_buf->len_begin]);
  #endif
  for(i = 0; i < len; i++, j++){
    if(j  >= MAX_IEC_LARGE_OUT_BUF_LEN) {
      j = 0;
...
void wrt_out_buf(iec3_large_out_buf_t *iec3_buf, iec3_small_out_buf_t *iec3_out_buf){
  ...
  #ifdef DEBUG_IEC870_3
  printf("3: begin of cile len buffer: %d ,data in buffer: %x\n", iec3_buf->len_begin, iec3_buf->len[iec3_buf->len_begin]);
  printf("4: end of cile len buffer: %d ,data in buffer: %x\n", iec3_buf->len_end, iec3_buf->len[iec3_buf->len_end]);
  printf("5: Number of packages in large buffer: %d \n", iec3_buf->num);
  #endif
  if(iec3_buf->num !=0 ){
    ...
    iec3_buf->begin = j;
    iec3_buf->len_begin++;
    if(iec3_buf->len_begin >= MAX_IEC_LARGE_OUT_BUF_LEN) iec3_buf->len_begin = 0;
    iec3_buf->num--;  //Уменьшаем количество пакетов буфере на 1  и размер буфера
    
  }else iec3_out_buf->len = 0;
  #ifdef DEBUG_IEC870_3
  if(iec3_out_buf->len > 6){
    printf("6: begin of cile len buffer: %d ,data in buffer: %x\n", iec3_buf->len_begin, iec3_buf->len[iec3_buf->len_begin]);
    printf("7: end of cile len buffer: %d ,data in buffer: %x\n", iec3_buf->len_end, iec3_buf->len[iec3_buf->len_end]);
    printf("8: Write to small iec transmitter buffer message with len  %d: ", iec3_out_buf->len);
...
void iec3_init(kp_cfg_t *kp_cfg, iec3_large_out_buf_t *iec3_buf, iec3_small_out_buf_t *iec3_out_buf){
  ...
  for(i = 0; i < MAX_IEC_LARGE_OUT_BUF_LEN; i ++){
    iec3_buf->data[i] = 0;
    iec3_buf->len[i] =0;
  }
  iec3_out_buf->len = 0;
  for(i = 0; i < MAX_IEC_SMALL_OUT_BUF_LEN; i ++) iec3_out_buf->data[i] = 0;
...
unsigned int iec3_convert_plc2kltm(char *plc_data, unsigned int len,  kp_cfg_t *kp_cfg, iec3_large_out_buf_t *iec3_buf){
  ...
  unsigned char temp_pack[MAX_IEC_SMALL_OUT_BUF_LEN]; //пакет будет формироваться здеся
  ...
  do{
    ...
    copy_plc_sig2iec3(kp_cfg, pos_plc_data, &plc_sig_len, temp_sign, &temp_sig_len);//преобразовываем данные из контроллера в формат блока передачи
